...
# Fitted hazard and cumulative hazard
fithHO <- Vectorize(function(t) hHO(t,exp(OPT$par[1]), exp(OPT$par[2]), exp(OPT$par[3]), h00, r00))
fitchHO <- Vectorize(function(t) chHO(t,exp(OPT$par[1]), exp(OPT$par[2]), exp(OPT$par[3]), h00, r00))


# Comparison: hazard functions
curve(fithHO, 0, max(times), xlim= c(0,max(times)), ylim = c(0,0.125), lwd = 2,
...
curve(fitchHO, 0, max(times), ylim = c(0,0.125), lwd = 2,
     xlab = "Time", ylab = "Hazard", main = "Hazard-Response ODE", cex.axis = 1.5, cex.lab = 1.5)
