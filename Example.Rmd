...
The harmonic oscillator hazard model is defined through the second order ODE:
\begin{equation}
h''(t) + 2 \eta w_0 h'(t) + w^2_0 (h(t) - h_b) = 0,
\label{eq:harmonic_osc}    
\end{equation}
where $w_0 > 0$ is called the "natural frequency", $\eta >0$ is the "damping coefficient". The resulting model has closed-form hazard and cumulative hazard functions, facilitating likelihood and Bayesian inference on the parameters, as detailed in @christen:2024.
